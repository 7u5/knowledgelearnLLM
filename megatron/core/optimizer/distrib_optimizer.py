        ...
        for gbuf_idx, gbuf_range_maps in enumerate(self.gbuf_ranges):
            ...
            for dtype, gbuf_range_map_for_all_buckets in gbuf_range_maps.items():
                ...
                if data_parallel_rank == 0:
                    keys = ("param", "exp_avg", "exp_avg_sq") if get_args().optimizer_selection != 'muon' else ("param", "moment1", "moment2")
                    world_tensors = {
                        key: torch.zeros(
    ...
    def load_parameter_state_from_dp_zero_legacy(self, state_dict):
        ...
        for gbuf_idx, gbuf_range_maps in enumerate(self.gbuf_ranges):
            for dtype, gbuf_range_map_for_all_buckets in gbuf_range_maps.items():
                ...
                keys = ("param", "moment1", "moment2") if get_args().optimizer_selection == "muon" else ("param", "exp_avg", "exp_avg_sq")
                for key in keys:
