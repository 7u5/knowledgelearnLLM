...
class GPTModel(MegatronCoreGPTModel):
    ...
    def __init__(self,
        ...
        self.nextlp_process = False
        if self.config.next_latent_prediction_steps is not None and self.config.next_latent_prediction_steps > 0:
            from mindspeed_llm.core.transformer.next_latent_prediction import NextLatentPredictionBlock
            self.nextlp_process = True
            self.nextlp = NextLatentPredictionBlock(config=self.config, )
    ...
    def forward(self,
        ...
        if self.nextlp_process:
            hidden_states = self.nextlp(
                input_ids=input_ids,
                position_ids=position_ids,
                hidden_states=hidden_states,
                loss_mask=loss_mask,
