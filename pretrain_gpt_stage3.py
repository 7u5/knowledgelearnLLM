...
def model_provider(pre_process=True, post_process=True) -> Union[GPTModel, megatron.legacy.model.GPTModel]:
    ...
    if 'global_teacher1' not in globals():
        global_teacher1 = None
    if 'global_teacher2' not in globals():
    ...
    if not args.use_legacy_models:
        ...
        # Stage3 student: needs LM head (post_process=True) but will take decoder_input directly.
        model = GPTModel(
            config=config,
...
def _ensure_teachers_initialized():
    ...
    def _load_teacher(load_arg_name, pre_process, post_process, config):
        ...
        if not hasattr(args, load_arg_name) or getattr(args, load_arg_name) is None:
            raise ValueError(f"--{load_arg_name.replace('_','-')} must be specified for stage3 training")
...
def _lm_loss_reduce(per_token_loss: torch.Tensor, loss_mask: torch.Tensor):
    args = get_args()
    loss_mask = loss_mask.view(-1).float()
...
def forward_step_stage3(data_iterator, model: GPTModel):
    args = get_args()
    timers = get_timers()
    ...
    with torch.no_grad():
        ...
        h2 = h2.detach()
        if args.use_teacher_embedding_input:
            teacher_embedding = t1.embedding(input_ids=tokens, position_ids=position_ids).detach()

    decoder_input = h2
    if teacher_embedding is not None:
        if args.teacher_embedding_scale != 1.0:
            teacher_embedding = teacher_embedding * args.teacher_embedding_scale
        decoder_input = decoder_input + teacher_embedding
    ...
    lm_labels = tokens[:, 1:].contiguous()
    logits = logits[:, :-1, :].contiguous()
    per_token_ce = torch.nn.functional.cross_entropy(
        logits.view(-1, vocab_size), lm_labels.view(-1), reduction='none'
    ).view_as(lm_labels)
    return per_token_ce, partial(_lm_loss_reduce, loss_mask=loss_mask[:, 1:])
...
def _extra_args_provider(parser):
    ...
    parser.add_argument('--use-teacher-embedding-input', action='store_true',
                        help='If set, sum teacher embedding output with h2 before feeding the student model.')
    parser.add_argument('--teacher-embedding-scale', type=float, default=1.0,
                        help='Scale applied to teacher embedding when --use-teacher-embedding-input is enabled.')
    return parser
